...
function prg_success_redirect(string $tab, int $ma_dept_id = 0, string $message = 'Saved.'): void {
    ...
    if ($tab === 'mutual_aid' && $ma_dept_id > 0) {
        $qs .= '&ma_dept_id=' . $ma_dept_id;
    }
    header('Location: department_settings.php' . $qs);
    exit;
}
        ...
        if ($ma_dept_name === '' || $ma_short === '') {
            ...
            if (!$error && $newMutualDeptId > 0) {
                ...
                if ($stmt2->execute()) {
                    ...
                    prg_success_redirect('mutual_aid', (int)$selectedMutualDeptId, $flash);
        ...
        <form method="post">
          ...
          <div class="row g-3">
            ...
            <div class="col-12">
              ...
              <a class="btn btn-outline-secondary btn-lg" href="incidents.php?skip_onboard=1">Cancel</a>
            </div>
          </div>
        </form>
      </div>
  ...
  <?php elseif ($activeTab === 'mutual_aid'): ?>
    ...
    <div class="card shadow-sm">
      ...
      <div class="card-body">
        ...
        <?php else: ?>
          <form method="post" id="maAppForm" class="row g-3 align-items-end">
            <input type="hidden" name="action" value="save_ma_apparatus">
            <input type="hidden" name="ma_mutual_dept_id" id="ma_mutual_dept_id" value="<?= (int)$selectedMutualDeptId ?>">
            <input type="hidden" name="app_id" id="ma_app_id" value="0">

            <div class="col-md-6">
              <label class="form-label">Apparatus Label *</label>
              <input class="form-control" id="ma_apparatus_label" name="apparatus_label" required placeholder="Eng 1, Truck 8, Tanker 52">
            </div>

            <div class="col-md-3">
              <label class="form-label">Apparatus Type *</label>
              <select class="form-select" id="ma_apparatus_type_id" name="apparatus_type_id" required>
            ...
            <div class="col-md-1">
              <label class="form-label">Sort</label>
              <input class="form-control" id="ma_app_sort_order" type="number" name="sort_order" value="0">
            </div>
            ...
            <div class="col-12">
              <button class="btn btn-primary" id="maAppSubmitBtn" type="submit">Add Apparatus</button> <button class="btn btn-outline-secondary d-none" id="maAppCancelEdit" type="button">Cancel Edit</button>
            </div>
          </form>
                ...
                <?php else: ?>
                  <?php foreach ($maApparatus as $a): ?>
                    <tr>
                      ...
                      <td>
                        <button class="btn btn-sm btn-outline-primary me-1 btnMaAppEdit" type="button"
                          data-app-id="<?= (int)$a['id'] ?>"
                          data-label="<?= e((string)$a['apparatus_label']) ?>"
                          data-type-id="<?= (int)$a['apparatus_type_id'] ?>"
                          data-sort="<?= (int)$a['sort_order'] ?>"
                          data-active="<?= (int)$a['is_active'] ?>">Edit</button>
    ...
    <script>
      ...
      // Mutual Aid Apparatus: inline edit -> fills the form above and switches button text to "Save Changes"
      function resetMaAppForm() {
        const idEl = document.getElementById('ma_app_id');
        const labelEl = document.getElementById('ma_apparatus_label');
        const typeEl = document.getElementById('ma_apparatus_type_id');
        const sortEl = document.getElementById('ma_app_sort_order');
        const activeEl = document.getElementById('ma_app_active');
        const submitBtn = document.getElementById('maAppSubmitBtn');
        const cancelBtn = document.getElementById('maAppCancelEdit');
        if (!idEl || !labelEl || !typeEl || !sortEl || !activeEl || !submitBtn) return;
      ...
      document.querySelectorAll('.btnMaAppEdit').forEach(function(btn){
        btn.addEventListener('click', function(){
          const idEl = document.getElementById('ma_app_id');
          const labelEl = document.getElementById('ma_apparatus_label');
          const typeEl = document.getElementById('ma_apparatus_type_id');
          ...
          idEl.value = btn.getAttribute('data-app-id') || '0';
          labelEl.value = btn.getAttribute('data-label') || '';
          typeEl.value = btn.getAttribute('data-type-id') || '0';
          sortEl.value = btn.getAttribute('data-sort') || '0';
          activeEl.checked = (btn.getAttribute('data-active') === '1');

          submitBtn.textContent = 'Save Changes';
          cancelBtn?.classList.remove('d-none');
          document.getElementById('maAppForm')?.scrollIntoView({behavior:'smooth', block:'start'});
        });
      });

      document.getElementById('maAppCancelEdit')?.addEventListener('click', function(){
        resetMaAppForm();
      });
